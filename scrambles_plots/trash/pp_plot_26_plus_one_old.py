...
OS_no_jumps_2 = []
for i in range(rang):
    OS_file_path = f'/fred/oz002/vdimarco/jumps/jumps_or_no_jumps/sims/30psr_100reals_final-noDM-with_jumps/output/real_{i}/OS.txt'
    with open(OS_file_path, 'r') as OS_file:
        contents = OS_file.read()
        data_dict = json.loads(contents)
        OS_value  = data_dict.get('OS')
...
for i in range(rang):
    ...
    scrambles_file_path_nojumps_2 = f'/fred/oz002/vdimarco/jumps/jumps_or_no_jumps/sims/30psr_100reals_final-noDM-with_jumps/output/real_{i}/scrambles.txt'
...
plt.plot(x, p_est_nojumps, label='no jumps new')
plt.plot(x, p_est_nojumps_2, label='no jumps old')
